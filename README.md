...
Thanks to Solidity, creating contracts is better than ever before! The Ethereum-compatible blockchain helps us to connect financial institutions, build smart contracts to automate some company finances to make everyone's lives easier, increase transparency, and make accounting and auditing practically automatic! The following 3 `ProfitSplitter` contracts will do several things:
...
Take note of the initial balances of your wallets, this will allow you to see if your code is distributing deposits properly.

In the `Deploy` tab in Remix, deploy the contract to your local Ganache chain by connecting to `Injected Web3` and ensuring MetaMask is pointed to `localhost:8545`.

You will need to fill in the constructor parameters with your designated `employee` addresses.

![Level One Test](Images/level_one_test.png)

Test the `deposit` function by sending various values. Keep an eye on the `employee` balances as you send different amounts of Ether to the contract and ensure the logic is executing properly.

![Level One Transaction](Images/level_one_tx.png)

![Level One Results](Images/level_one_result.png)

If done correctly, as shown above, the deposit amount should be split equally into thirds between the targeted accounts.
...
Using the previous results as the starting point, we can test to see if the code correctly distributes the desired values

![Level One Test](Images/level_two_test.png)

![Level One Transaction](Images/level_two_tx.png)

![Level One Transaction](Images/level_two_result.png)

When done correctly, as shown above, the contract should distribute the desired deposit amount according to respective tier level.
---
...
After switching MetaMask to Kovan/Ropsten, deploy the contracts as before and copy/keep a note of their deployed addresses. The transactions will also be in your MetaMask history, and on the blockchain permanently to explore later.
